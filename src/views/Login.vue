<template>
  <div class="min-h-screen flex bg-gray-50 dark:bg-[rgb(16,22,34)] font-display">
    ...
    <div class="w-full lg:w-1/2 flex flex-col items-center justify-center p-6 sm:p-12 relative">
      <div class="w-full max-w-md space-y-8">
        <div class="text-center lg:text-left">
          ...
          <p class="mt-2 text-sm text-gray-600 dark:text-slate-400">
            {{ esRegistro ? 'Registra tus datos para comenzar' : 'Ingresa tus credenciales' }}
          </p>
        </div>
        ...
        <div class="mt-8 space-y-6">
          ...
          <div v-if="mensaje" :class="`p-3 rounded-lg text-sm text-center font-medium ${tipoMensaje === 'error' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`">
            {{ mensaje }}
          </div>
          ...
          <div class="space-y-4">
            ...
            <div class="relative py-2">
              ...
              <div class="relative flex justify-center text-sm"><span class="px-2 bg-gray-50 dark:bg-[rgb(16,22,34)] text-gray-500">O</span></div>
            </div>

            <p class="text-center text-sm text-gray-600 dark:text-slate-400">
              {{ esRegistro ? '¿Ya tienes cuenta?' : '¿No tienes cuenta?' }}
              <button @click="cambiarModo" class="font-semibold text-blue-600 hover:text-blue-500 ml-1">
